...
+predInt m (pos (suc n)) = (predSuc (m + pos n)) ∙ (cong-Str (_+_ m) (sym (predSuc (pos n))))
...
+sucInt m (negsuc (suc n)) = (sucPred (m +negsuc n)) ∙ (cong-Str (_+_ m) (sym (sucPred (negsuc n))))
...
negsuc0+ (pos (suc n)) = (sym (sucPred (pos n))) ∙ (cong-Str sucInt (negsuc0+ _))
...
isEquivAddInt : (m : Int) → isEquiv (λ n → n + m)
isEquivAddInt = subst-Str (λ add → (m : Int) → isEquiv (λ n → add n m)) +'≣+ isEquivAddInt'
-}
