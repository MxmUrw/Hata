...
module Verification.Old.Core.Category.Monad.Instance.Coproduct where
...
module _ {𝒞 : Category 𝑖} {{_ : hasCoproducts 𝒞}} where
  ...
  -- instance IFunctor:Either = #openstruct Functor:Either
    ...
    instance
      IMonad:Either : ∀{x : ⟨ 𝒞 ⟩} -> IMonad (Functor:Either x)
    ...
    module _ (x : ⟨ 𝒞 ⟩) (T : Monad 𝒞) where
      ...
      module VCCMIC-1 where
        private
          instance _ = of Monad:EitherT
          instance _ = of Functor:EitherT
        lem-1 : ∀{a b c : ⟨ 𝒞 ⟩} -> (f : Hom {{of 𝒞 ⌄ T}} ` a ` ` b `) -> (g : Hom {{of 𝒞 ⌄ Monad:EitherT}} ` b ` ` c `)
             -> (⟨ f ⟩ ◆ map ι₁) ◆ map ⟨ g ⟩ ◆ join ≣ ⟨ f ⟩ ◆ map ⟨ g ⟩ ◆ join
        lem-1 f g = (⟨ f ⟩ ◆ map ι₁) ◆ map ⟨ g ⟩ ◆ (map [ ι₀ ◆ return , id ] ◆ join)

                    ≣⟨ assoc-l-◆ ◈ refl ⟩

                    ⟨ f ⟩ ◆ (map ι₁ ◆ map ⟨ g ⟩) ◆ (map [ ι₀ ◆ return , id ] ◆ join)

                    ≣⟨ refl ◈ (functoriality-◆ {{of ⟨ T ⟩}} ⁻¹) ◈ refl ⟩

                    ⟨ f ⟩ ◆ (map (ι₁ ◆ map ⟨ g ⟩)) ◆ (map [ ι₀ ◆ return , id ] ◆ join)

                    ≣⟨ refl ◈ (functoriality-≣ (naturality {{INatural:return}} _)) ◈ refl ⟩

                    ⟨ f ⟩ ◆ (map (⟨ g ⟩ ◆ ι₁)) ◆ (map [ ι₀ ◆ return , id ] ◆ join)

                    ≣⟨ refl ◈ (functoriality-◆ {{of ⟨ T ⟩}}) ◈ refl ⟩

                    ⟨ f ⟩ ◆ (map ⟨ g ⟩ ◆ map ι₁) ◆ (map [ ι₀ ◆ return , id ] ◆ join)
                    ...
                    ⟨ f ⟩ ◆ (map ⟨ g ⟩ ◆ ((map (ι₁ ◆ [ ι₀ ◆ return , id ])) ◆ join))

                    ≣⟨ refl ◈ (refl ◈ (functoriality-≣ (reduce-+-₁) ◈ refl)) ⟩

                    ⟨ f ⟩ ◆ (map ⟨ g ⟩ ◆ ((map id) ◆ join))

                    ≣⟨ refl ◈ (refl ◈ (functoriality-id ◈ refl)) ⟩
      ...
      𝑲+ : 𝒞 ⌄ T ⟶ 𝒞 ⌄ (Monad:EitherT)
      ⟨ 𝑲+ ⟩ a = ` ⟨ a ⟩ `
      IFunctor.map (of 𝑲+) f = ` ⟨ f ⟩ ◆ map {{of ⟨ T ⟩}} return `
      IFunctor.functoriality-id (of 𝑲+) = refl
      IFunctor.functoriality-◆ (of 𝑲+) {f = f} {g} =
        let ET = Functor:EitherT
            f = ⟨ f ⟩
            g = ⟨ g ⟩
            P : (f ◆ map g ◆ join) ◆ map ι₁ ≣ (f ◆ map ι₁) ◆ (map {{of ET}} (g ◆ map ι₁)) ◆ (map ([ ι₀ ◆ return , id ]) ◆ join)
            P = ((f ◆ map g) ◆ join) ◆ map ι₁
                ...
                ≣⟨ refl ◈ (refl ◈ (naturality {{INatural:join}} _)) ⟩

                f ◆ (map g ◆ (map (map ι₁) ◆ join))

                ≣⟨ refl ◈ (refl ◈ (functoriality-≣ (unit-r-◆ ⁻¹) ◈ refl)) ⟩

                f ◆ (map g ◆ (map (map ι₁ ◆ id) ◆ join))

                ≣⟨ refl ◈ (refl ◈ (functoriality-≣ (refl ◈ (reduce-+-₁ ⁻¹)) ◈ refl)) ⟩
        ...
        in P
      IFunctor.functoriality-≣ (of 𝑲+) p = p ◈ refl
