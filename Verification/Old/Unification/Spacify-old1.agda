...
module _ {K : 𝒰 𝑖} (T : Functor `(IdxSet K 𝑖)` `(IdxSet K 𝑖)`) {{_ : IMonad T}} {{_ : IRecAccessible-Monad T}} where
  ...
  ⟨ Mod X k ⟩ = ∑ λ j -> Hom {{of 𝔇}} j k ×-𝒰 (𝟚-𝒰 +-𝒰 (⟨ ⟨ T ⟩ X ⟩ j))
  of Mod X k = {!!}
  ...
  private
    module _ {X : IdxSet K 𝑖} where
      ...
      ν-impl : {j : K} {k : K} -> (p : QPath {{of Q}} j k) -> 𝟚-𝒰 +-𝒰 ⟨ ⟨ T ⟩ X ⟩ j -> ⟨ Mod X k ⟩
      ν-impl p (left x) = _ , id , left ₀
      ν-impl (last nothing) (right x) = _ , id , left ₀
      ν-impl (last (just e)) (right x) with split-+ (⟨ ⟨ decompose ⟩ ⟩ _ x)
      ... | left _        = _ , some (last (just e)) , right x
      ... | just (xs , _) = _ , id , (xs _ e)
      ν-impl (nothing ∷ p) (right x) = _ , id , left ₀
      ν-impl (just e ∷ p) (right x) with split-+ (⟨ ⟨ decompose ⟩ ⟩ _ x)
      ... | left _        = _ , some (just e ∷ p) , right x
      ... | just (xs , _) = ν-impl p (xs _ e)
      ...
      write e (_ , p , x) = (_ , p ◆ ` e ` , x)

      write-comm-impl : {j k l : K} -> (e : Edge {{of Q}} k l) -> (p : QPath {{of Q}} j k) -> (x : 𝟚-𝒰 +-𝒰 ⟨ ⟨ T ⟩ X ⟩ j) -> ν (write e (ν-impl p x)) ≡ ν-impl (comp-QPath p (last e)) x
      ...
      -- write-comm-impl e' (last e) x with (ν-impl-1 _ e x)
      -- ... | just X = {!!}
      -- ... | left x₁ = {!!}
      -- with split-+ (ν-impl-1 _ e x)
      -- ... | left x₂ = {!!}
      -- ... | just x₂ = {!!}
      -- write-comm-impl e' (e ∷ p) x = {!!}
