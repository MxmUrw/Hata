...
module Verification.Unification.Instance.SignatureZ3 where
  ...
  module _ (σ : Signature) (isInhabited:σ : isInhabited-Sig σ) where
    ...
    module _ {{_ : ∀{n k} {ks : Vec K (suc n)} -> IDiscreteStr (σ k ks)}} where
      ...
      data isDecomposableP-Term {k : K} {X : K -> 𝒰₀} : Term σ X k -> 𝒰₀ where
        isTe : ∀{ks : Vec K (suc n)} -> (s : σ k ks) -> (ts : Termsᵥ σ X ks) -> isDecomposableP-Term (te s ts)
      ...
      module _ {V : K -> 𝒰₀} where
        ...
        lookup-Term-try-⊏ : ∀{n₁ n₂ : ℕ} {ks₁ : Vec K (suc n₁)} {ks₂ : Vec K (suc n₂)} (s₁ : σ k ks₁) (s₂ : σ k ks₂) (i : Fin-R (suc n₂)) (ts : Termsᵥ σ V ks₁)
                          -> lookup-Term-try s₁ s₂ i (forget-Terms ts) ⊏ te s₁ ts
        ...
        ... | yes refl-StrId with (s₁ ≟-Str s₂)
        ... | no ¬p = fail (λ ())
        ... | yes refl-StrId = te (lem-05.proof i (forget-Terms ts))
      ...
      module _ {X Y : IdxSet K ⊥} where
        naturality-lookup-Term-try : (f : X ⟶ Y) {k : K} -> ∀ {n n₁}
                            ...
                            (ts  : Termsᵥ σ (⟨ X ⟩) ks₁) →
                            (i : Fin-R (suc n₁)) ->
                            (map-Term ⟨ f ⟩) (lookup-Term-try s₁ s₂ i (forget-Terms ts)) ≡
                                  lookup-Term-try s₁ s₂ i (forget-Terms (map-Termsᵥ ⟨ f ⟩ ts))
        ...
        ... | yes refl-StrId with (s₁ ≟-Str s₂)
        ... | no ¬p = refl
        ... | yes refl-StrId = naturality-lookup-Term ⟨ f ⟩ i (forget-Terms ts) ∙ cong (lookup-Term i) (commutes:map∣forget-Terms ⟨ f ⟩ ts)

        -- λ j -> ((naturality-lookup-Term ⟨ f ⟩ i ts j))
...
-}
      ...
      decomp (te s₁ ts) (edge i s₂) = just (lookup-Term-try s₁ s₂ i (forget-Terms ts))
      ...
      module lem-25 {X Y : K -> 𝒰₀} (f : ∀{k} -> X k ⟶ Term σ Y k) where
          ...
          P3 :             ∀ {n₁} -> ∀ {ks₁ : Vec K (suc n₁)} ->
                            ...
                            (i : Fin-R (suc n₁)) ->
                            isFail-Term (join-Term (map-Term f (lookup-Term i (forget-Terms ts))))
          P3 s ts F i =
            let ts' = forget-Terms ts
                t = lookup-Term i ts'
                Q0 : t ⊏-Terms ts'
                ...
                Q3 : join-Terms (map-Terms f ts') ≡ (join-Termsᵥ (map-Termsᵥ f ts))
                Q3 = join-Terms (map-Terms f (forget-Terms ts)) ≡[ i ]⟨ join-Terms (commutes:map∣forget-Terms f ts i) ⟩
                     join-Terms (forget-Terms (map-Termsᵥ f ts)) ≡⟨ split:join∣forget-Terms _ ⁻¹ ⟩
                     join-Termsᵥ (map-Termsᵥ f ts)                ∎

                Q4 : join-Term (map-Term f t) ⊏-Terms (join-Termsᵥ (map-Termsᵥ f ts))
                Q4 = transport (λ j -> join-Term (map-Term f t) ⊏-Terms Q3 j) Q2

                Q5 : isFail-Term (join-Term (map-Term f t))
                Q5 = fail⊏-Terms Q4 F
          ...
          P2 :             ∀ {n₁ n₂} -> ∀ {ks₁ : Vec K (suc n₁)} {ks₂ : Vec K (suc n₂)} ->
                            ...
                            (i : Fin-R (suc n₂)) ->
                            (isFail-Terms (join-Termsᵥ (map-Termsᵥ f ts))) ->
                            (join-Term (map-Term f (lookup-Term-try s₁ s₂ i (forget-Terms ts)))) ≡ fail
          ...
          P4 :             ∀ {n₁ n₂} -> ∀ {ks₁ : Vec K (suc n₁)} {ks₂ : Vec K (suc n₂)} ->
                            ...
                            (i : Fin-R (suc n₂)) ->
                            -- (isFail-Terms (join-Termsᵥ (map-Termsᵥ f ts))) ->
                            (join-Term (map-Term f (lookup-Term-try s₁ s₂ i (forget-Terms ts)))) ≡ lookup-Term-try s₁ s₂ i (join-Terms (map-Terms f (forget-Terms ts)))
          ...
          ... | yes refl-StrId =
            let ts' = (forget-Terms ts)
                Q0 = join-Term (map-Term f (lookup-Term i ts')) ≡⟨ cong join-Term (naturality-lookup-Term f i ts') ⟩
          ...
          proof (edge i s₂) (te s₁ ts) P with split-+-Str (reduce-Terms (join-Termsᵥ (map-Termsᵥ f ts)))
          ... | left (aF , P') = cong right (P2 s₁ s₂ ts i aF)
          ... | just ((ts' , ts'P) , P') =
            let Q0 : join-Terms (map-Terms f (forget-Terms ts)) ≡ forget-Terms ts'
                Q0 = join-Terms (map-Terms f (forget-Terms ts)) ≡[ i ]⟨ join-Terms (commutes:map∣forget-Terms f ts i) ⟩
                     join-Terms (forget-Terms (map-Termsᵥ f ts)) ≡⟨ split:join∣forget-Terms _ ⁻¹ ⟩
                     join-Termsᵥ (map-Termsᵥ f ts)               ≡⟨ ts'P ⁻¹ ⟩
                     forget-Terms ts' ∎

                Q1 : (join-Term (map-Term f (lookup-Term-try s₁ s₂ i (forget-Terms ts)))) ≡ lookup-Term-try s₁ s₂ i (join-Terms (map-Terms f (forget-Terms ts)))
                Q1 = P4 s₁ s₂ ts i

                Q2 : (join-Term (map-Term f (lookup-Term-try s₁ s₂ i (forget-Terms ts)))) ≡ lookup-Term-try s₁ s₂ i (forget-Terms ts')
                Q2 = transport (λ j -> (join-Term (map-Term f (lookup-Term-try s₁ s₂ i (forget-Terms ts)))) ≡ lookup-Term-try s₁ s₂ i (Q0 j)) Q1
      ...
      module lem-60 {k} {X : K -> 𝒰₀} where
        ...
        nofailEdge : ∀{ks : Vec K n} -> (ts : Termsᵥ σ X ks) -> ∑ λ j -> ¬ isFail-Term (lookup-Term j (forget-Terms ts))
        nofailEdge (te s ts₁ ∷ ts) = zero , λ {()}
        nofailEdge (var x ∷ ts) = zero , λ {()}
        nofailEdge (fail∷ ts) = let j , jF = nofailEdge ts in suc j , jF

        -- nofailEdge2 : ∀{n₁ n₂}∀{ks : Vec K (suc n)} -> ∀{k} -> (s : σ k ks) -> (ts : Termsᵥ σ X ks)
        ...
        nofailEdge2 :  ∀ {n₁ n₂} -> ∀ {ks₁ : Vec K (suc n₁)} {ks₂ : Vec K (suc n₂)} ->
             ...
             (pks : PathP (λ i -> Vec K (suc (pn i))) ks₁ ks₂)
             (ps : PathP (λ i -> σ k (pks i)) s₁ s₂)
              -> ∑ λ j -> ¬ isFail-Term (lookup-Term-try s₁ s₂ j (forget-Terms ts))
        ...
        compare (fail) (te {n₂} {ks₂} s₂ ts₂) (isFail) (isTe s₂ ts₂) Pd =
          let j , Fj = nofailEdge2 s₂ s₂ ts₂ refl refl refl
          in left (_ , edge j s₂ ,
                  λ p -> let Q1 = isInjective:right p
                             Q2 = fail
                         in P0 Q2 Fj Q1
                  )
        compare (te {n₁} {ks₁} s₁ ts₁) (fail) (isTe s₁ ts₁) (isFail) Pd =
          let j , Fj = nofailEdge2 s₁ s₁ ts₁ refl refl refl
          in left (_ , edge j s₁ ,
                  λ p -> let Q1 = isInjective:right p
                             Q2 = fail
        ...
        compare (te {n₁} {ks₁} s₁ ts₁) (te {n₂} {ks₂} s₂ ts₂) (isTe s₁ ts₁) (isTe s₂ ts₂) Pd with n₁ ≟-Str n₂
        ... | no ¬p =
          let j , Fj = nofailEdge2 s₂ s₂ ts₂ refl refl refl
          in left (_ , edge j s₂ ,
                  λ p -> let Q1 = isInjective:right p
                             Q2 = P1 s₁ s₂ (forget-Terms ts₁) ¬p j
                         in P0 Q2 Fj Q1
                  )
        ... | yes refl-StrId with (ks₁ ≟-Str ks₂)
        ... | no ¬p =
          ...
          in left (_ , edge j s₂ ,
                  λ p -> let Q1 = isInjective:right p
                             Q2 = P2 s₁ s₂ (forget-Terms ts₁) refl (λ {p -> ¬p (≡→≡-Str p)}) j
                         in P0 Q2 Fj Q1
                  )
        ... | yes refl-StrId with (s₁ ≟-Str s₂)
        ... | no ¬p =
          ...
          in left (_ , edge j s₂ ,
                  λ p -> let Q1 = isInjective:right p
                             Q2 = P3 s₁ s₂ (forget-Terms ts₁) refl refl (λ {p -> ¬p (≡→≡-Str p)}) j
                         in P0 Q2 Fj Q1
                  )
        ... | yes refl-StrId =
          let Q0 : (j : Fin-R (suc n₁)) → lookup-Term j (forget-Terms ts₁) ≡ lookup-Term j (forget-Terms ts₂)
              Q0 j = eval-lookup-Term s₁ s₁ (forget-Terms ts₁) j j refl refl refl refl ⁻¹
                     ∙ isInjective:right (Pd (edge j s₁))
                     ∙ eval-lookup-Term s₂ s₂ (forget-Terms ts₂) j j refl refl refl refl
              Q1 : forget-Terms ts₁ ≡ forget-Terms ts₂
              Q1 = (P4 (forget-Terms ts₁) (forget-Terms ts₂) Q0)
              Q2 : ts₁ ≡ ts₂
      ...
      module lem-70 {X : K -> 𝒰₀} where
        proof : ∀{k j} -> (x : Term σ X k) -> (y : Term σ X j)
              ...
              -> x ⊏ y
        proof .(lookup-Term-try s x₁ i (forget-Terms ts)) (te s ts) (P , edge i x₁ , refl-StrId) = lookup-Term-try-⊏ s x₁ i ts
        proof .fail (te s ts) (P , fail , refl-StrId) = fail P
