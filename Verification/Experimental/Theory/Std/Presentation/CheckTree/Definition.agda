...
module Verification.Experimental.Theory.Std.Presentation.CheckTree.Definition where
...
module _ {A : 𝒰 𝑖} {{_ : isSet-Str A}} where
  rem₂ : ∀(as : 人List A) -> {a1 a2 : A} -> (a1p : as ∍ a1) -> (a2p : as ∍ a2) -> (a1p ≠-∍ a2p) -> 人List A
  rem₂ as a1p a2p 1≠2 = (as \\ a2p) \\ (skip-∍ a1p a2p 1≠2)

module _ {ℬ : 𝒰 𝑖} {{_ : isCategory {𝑗} ℬ}} {{_ : isSet-Str ℬ}} where
  data ArrowTree : (人List ℬ) -> ℬ -> 𝒰 (𝑖 ､ 𝑗) where
  ...
  data ArrowForrest : (人List ℬ) -> (人List ℬ) -> 𝒰 (𝑖 ､ 𝑗) where
  ...
  data Link-AF : {as0 as bs : 人List ℬ} -> ∀{a b} -> (t : ArrowTree as0 b) (ts : ArrowForrest as bs) -> (as0 ∍ a) -> (as ∍ a) -> (bs ∍ b) -> 𝒰 (𝑖 ､ 𝑗) where
  ...
  record MiniLink-AF {as bs : 人List ℬ} {a b} (ts : ArrowForrest as bs) (p0 : as ∍ a) (p1 : bs ∍ b) : 𝒰 (𝑖 ､ 𝑗) where
    constructor minilink
    field {sublist-Link} : 人List ℬ
  ...
  record MicroLink-AF {as bs : 人List ℬ} {b} (ts : ArrowForrest as bs) (p1 : bs ∍ b) : 𝒰 (𝑖 ､ 𝑗) where
  ...
  getLink : {as bs : 人List ℬ} -> ∀{a} (ts : ArrowForrest as bs) (p0 : as ∍ a) -> ∑ λ b -> ∑ λ (p1 : bs ∍ b) -> MiniLink-AF ts p0 p1
  ...
  treehom : {as0 : 人List ℬ} -> ∀{a b} -> (t : ArrowTree as0 b) -> (p : as0 ∍ a) -> a ⟶ b
  treehom (incl _) incl = id
  treehom (x ∧-⧜ (y , fy)) (right-∍ p) = treehom y p ◆ fy
  treehom ((x , fx) ∧-⧜ y) (left-∍ p) = treehom x p ◆ fx

  linkhom : {as bs : 人List ℬ} -> ∀{a b} -> {ts : ArrowForrest as bs} {p0 : as ∍ a} -> {p1 : bs ∍ b} -> (l : MiniLink-AF ts p0 p1) -> a ⟶ b
  linkhom (minilink sublist-member subtree-Link minilink-Link) = f minilink-Link
    where
      f : {as0 as bs : 人List ℬ} -> ∀{a b} -> {t : ArrowTree as0 b} {ts : ArrowForrest as bs} -> {q : as0 ∍ a} -> {p0 : as ∍ a} -> {p1 : bs ∍ b}
          -> Link-AF t ts q p0 p1 -> a ⟶ b
      f {t = t} (incl p) = treehom t p
      f (left-∍ l) = f l
      f (right-∍ l) = f l

  merge-AF : ∀{as bs : 人List ℬ}
        -> (ts : ArrowForrest as bs)
        -> {b1 b1' b2 b2' x : ℬ}
        -> {b1p : as ∍ b1} -> {b2p : as ∍ b2}
        -> {b1p' : bs ∍ b1'} -> {b2p' : bs ∍ b2'}
...
module _ {ℬ : 𝒰 𝑖} {{_ : isCategory {𝑗} ℬ}} {{_ : isSet-Str ℬ}} {F : Functor ′ ℬ ′ (𝐔𝐧𝐢𝐯 𝑙)} {{_ : isCheckingBoundary ′ ℬ ′ F}}
  ...
  -- data CheckStrategy (cs : 人List C) : 𝒰 𝑘 where

  CheckStrategy : 𝒰 _
  CheckStrategy = List (C × C)

  -- data CheckStep : {as bs cs : 人List ℬ} (ts : ArrowForrest as bs) (us : ArrowForrest as cs) -> 𝒰 (𝑖 ､ 𝑗) where
  --   incl : ∀{as bs : 人List ℬ}
  --        -> (ts : ArrowForrest as bs)
  --        -> {b1 b2 x : ℬ}
  --        -> (b1p : bs ∍ b1) -> (b2p : bs ∍ b2) -> (1≠2 : b1p ≠-∍ b2p)
  ...
  module _ {as : 人List ℬ} (initb : C -> ℬ) (initbo : ∀(c : C) -> as ∍ initb c) (initv : ∀(c : C) -> ⟨ F ⟩ (initb c)) where

    data CheckTree : (s : CheckStrategy) {bs : 人List ℬ} (ts : ArrowForrest as bs) -> 𝒰 (𝑘 ､ 𝑖 ､ 𝑗) where
  ...
  -- data Link-AF : {as bs : 人List ℬ} -> (ts : ArrowForrest as bs) -> ∀{a} -> (as ∍ a) -> (bs ∍ a) -> 𝒰 (𝑖 ､ 𝑗) where
  ...
  -- data CheckTree : (s : CheckStrategy) {as bs cs : 人List ℬ} (ts : ArrowForrest as bs) (us : ArrowForrest as cs) -> 𝒰 (𝑘 ､ 𝑖 ､ 𝑗) where
