    ...
    where
      ...
      mutual
        ...
        map-f₀ : ∀{𝔍 Δ Γ α} ->
                ...
                -- -> Hom (⟦ Γ ↷ (𝔍 ⋆ Δ) ⟧) (⟦ Γ ↷ α ⟧)
                -> Hom (⟦ Γ ⟧-LK ⊗ ⟦ (𝔍 ⋆ Δ) ⟧) (⟦ Γ ⟧-LK ⊗ ⟦ α ⟧)
        map-f₀ (meta {αs} {α}) = map (id , ⟨ unit-r-⋆ ⟩)

        map-f₀ {𝔍} {Δ} {Γ} {α} (app {𝔍₀} {𝔍₁} {𝔧 = 𝔧} t s) =
          let t' = map-f₀ t
              s' = map-f₀ s
              P =
                ⟦ Γ ⟧-LK ⊗ ⟦ (𝔍₁ ⋆ 𝔍₀) ⋆ Δ ⟧          ⟶⟨ map (id , ⟨ ≀⟦⟧≀ (assoc-l-⋆ {a = 𝔍₁} {𝔍₀} {Δ}) ⟩) ⟩
                ⟦ Γ ⟧-LK ⊗ ⟦ 𝔍₁ ⋆ (𝔍₀ ⋆ Δ) ⟧          ⟶⟨ id ⇃⊗⇂ ⟨ ⟦⊗⟧ {Γ = 𝔍₁} {Δ = 𝔍₀ ⋆ Δ} ⟩ ⟩
                ⟦ Γ ⟧-LK ⊗ (⟦ 𝔍₁ ⟧ ⊗ ⟦ 𝔍₀ ⋆ Δ ⟧)      ⟶⟨ ⟨ assoc-l-⋆ ⁻¹ ⟩ ⟩
                ⟦ Γ ⟧-LK ⊗ ⟦ 𝔍₁ ⟧ ⊗ ⟦ 𝔍₀ ⋆ Δ ⟧        ⟶⟨ id ⇃⊗⇂ ⟨ ≀⟦⟧≀ (unit-r-⋆ {a = 𝔍₁} ⁻¹) ⟩ ⇃⊗⇂ id ⟩
                ⟦ Γ ⟧-LK ⊗ ⟦ 𝔍₁ ⋆ [] ⟧ ⊗ ⟦ 𝔍₀ ⋆ Δ ⟧    ⟶⟨ s' ⇃⊗⇂ id ⟩
                ⟦ Γ ⟧-LK ⊗ ⟦ 𝔧 ⟧ ⊗ ⟦ 𝔍₀ ⋆ Δ ⟧    ⟶⟨ id ⇃⊗⇂ ⟨ ⟦⊗⟧ {Γ = 𝔍₀} {Δ = Δ} ⟩ ⟩
                ⟦ Γ ⟧-LK ⊗ ⟦ 𝔧 ⟧ ⊗ (⟦ 𝔍₀ ⟧ ⊗ ⟦ Δ ⟧)   ⟶⟨ ⟨ assoc-l-⋆ ⟩ ⟩
                ⟦ Γ ⟧-LK ⊗ (⟦ 𝔧 ⟧ ⊗ (⟦ 𝔍₀ ⟧ ⊗ ⟦ Δ ⟧))   ⟶⟨ id ⇃⊗⇂ ⟨ assoc-l-⋆ ⁻¹ ⟩ ⟩
                ⟦ Γ ⟧-LK ⊗ ((⟦ 𝔧 ⟧ ⊗ ⟦ 𝔍₀ ⟧) ⊗ ⟦ Δ ⟧)   ⟶⟨ id ⇃⊗⇂ (braid ⇃⊗⇂ id) ⟩
                ⟦ Γ ⟧-LK ⊗ ((⟦ 𝔍₀ ⟧ ⊗ ⟦ 𝔧 ⟧) ⊗ ⟦ Δ ⟧)   ⟶⟨ id ⇃⊗⇂ ⟨ assoc-l-⋆ ⟩ ⟩
                ⟦ Γ ⟧-LK ⊗ (⟦ 𝔍₀ ⟧ ⊗ (⟦ 𝔧 ⟧ ⊗ ⟦ Δ ⟧))   ⟶⟨ id ⇃⊗⇂ (id ⇃⊗⇂ (⟨ unit-r-⋆ ⁻¹ ⟩ ⇃⊗⇂ id) ) ⟩
                ⟦ Γ ⟧-LK ⊗ (⟦ 𝔍₀ ⟧ ⊗ (⟦ 𝔧 ∷ [] ⟧ ⊗ ⟦ Δ ⟧))   ⟶⟨ id ⇃⊗⇂ (id ⇃⊗⇂ ⟨ ⟦⊗⟧ {Γ = 𝔧 ∷ []} {Δ = Δ} ⁻¹ ⟩ ) ⟩
                ⟦ Γ ⟧-LK ⊗ (⟦ 𝔍₀ ⟧ ⊗ (⟦ 𝔧 ∷ Δ ⟧))       ⟶⟨ id ⇃⊗⇂ ⟨ ⟦⊗⟧ {Γ = 𝔍₀} {Δ = 𝔧 ∷ Δ} ⁻¹ ⟩ ⟩
                ⟦ Γ ⟧-LK ⊗ (⟦ 𝔍₀ ⋆ (𝔧 ∷ Δ) ⟧)             ⟶⟨ t' ⟩
                ⟦ Γ ⟧-LK ⊗ ⟦ α ⟧                        ⟶id
