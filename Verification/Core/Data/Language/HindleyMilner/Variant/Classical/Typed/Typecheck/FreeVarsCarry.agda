...
module Verification.Core.Data.Language.HindleyMilner.Variant.Classical.Typed.Typecheck.FreeVarsCarry where
...
record CtxTypingInstance {μs k} {Q : ℒHMQuant k} (Γ : ℒHMCtxFor Q μs) (te : UntypedℒHM k) : 𝒰₀ where
  ...
  field hasType : isTypedℒHM (metas ⊔ typeMetas ⊩ (Q , ctx) ⊢ typ) te
...
γ {μs = νs} Γ (app te se) =
  ...
  continue₀ ((νs₀ / νs₀ₓ ⊩ Γ₀ , α₀ , Γ<Γ₀ , hσ₀ , phσ₀ , Γ₀⊢α₀), Ω₀) =
    case (γ _ se) of
    {!!}
    continue₁ where

    -- continue₁ : InitialCtxTypingInstance Γ₀ se -> TypingDecision Γ (app te se)
    continue₁ : InitialCtxTypingInstance (Γ ⇃[ hσ₀ ]⇂-CtxFor) se -> TypingDecision Γ (app te se)
    continue₁ ((νs₁ / νs₁ₓ ⊩ Γ₁ , β₁ , Γ₀<Γ₁ , hσ₁ , phσ₁ , Γ₁⊢β₁), Ω₁) = {!!} where
