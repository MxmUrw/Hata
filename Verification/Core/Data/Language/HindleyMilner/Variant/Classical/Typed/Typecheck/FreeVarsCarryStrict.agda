  ...
  where
    ...
    success ((μs₁ₐ / μs₁ₓ ⊩ (α₁ ∷ Γ₁) , β₁ , α₀Γ₀<α₁Γ₁ , α₁Γ₁⊢β₁) , Ω) = {!!} , {!!} -- 𝑇 , isInitial:𝑇
      where
        ...
        ϕ' : μs₂ₐ ⊔ (μs₂ₓ ⊔ μs₁ₓ) ≅ (μs₁ₐ ⊔ μs₁ₓ)
        ϕ' = {!!} -- assoc-l-⊔-ℒHMTypes ∙-≅ (ϕ )
        ...
        lem-05 : isTypedℒHM (μs₂ₐ ⊔ (μs₂ₓ ⊔ μs₁ₓ) ⊩ (_ , Γ₂ ⇃[ ι₀ ]⇂ᶜ) ⊢ α₂ ⇒ β₂) (lam te)
        lem-05 = {!!}

        Γ<Γ₂ : Γ <Γ Γ₂
        Γ<Γ₂ = record { fst = σᵃᵤ₂ ; snd = {!!} }
        ...
        isInitial:𝑇 (μs₃ₐ / μs₃ₓ ⊩ Γ₃ , .(_ ⇒ _) , Γ<Γ₃ , lam {α = α₃} {β = β₃} Γ₃α₃⊢β₃) =
          record { tiSubₐ = σᵃ₂₃ ; tiSubₓ = σˣ₂₃ ; typProof = {!!} ; subProof = lem-20 }
...
{-
    ...
    -}
            ...
            ΩR = Ω (μs₂ₐ ⊔ μs₂ₓ / ⊥ ⊩ (α₂ ∷ Γ₂') , β₂' , {!!} , lem-9 )

            σᵃ₁₂ : μs₁ₐ ⟶ μs₂ₐ ⊔ μs₂ₓ
            σᵃ₁₂ = tiSubₐ ΩR
-}
            -- ΩR = Ω (μs₂ₐ / μs₂ₓ ⊩ (α₂ ∷ Γ₂) , β₂ , ? , ? )
            -- α₀Γ₀<α₂Γ₂ , Γ₂α₂⊢β₂)
